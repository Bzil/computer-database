...
<div class="container text-center">
    <ul class="pagination">
        ...
        <c:if test="${active > 1}">
            ...
            <li>
                <mylib:link body="&lsaquo;" target="dashboard" page="${active - 1}" size="${page.offset}" active="false"
                            search="${page.search}"></mylib:link>
            </li>
        </c:if>
        ...
        <c:if test="${active < page.pageNb }">
            <li>
                <mylib:link body="&rsaquo;" target="dashboard" page="${active +1}" size="${page.offset}" active="false"
                            search="${page.search}"></mylib:link>
            </li>
            <li>
                <mylib:link body="&raquo;" target="dashboard" page="${page.pageNb}" size="${page.offset}" active="false"
                            search="${search}"></mylib:link>
            </li>
        </c:if>
    ...
    <div class="btn-group btn-group-sm pull-right" role="group">
        <button type="button" class="btn btn-default"
                onclick="document.location.href='dashboard?id=${active}&size=10&search=${page.search}'">10
        </button>
        <button type="button" class="btn btn-default"
                onclick="document.location.href='dashboard?id=${active}&size=50&search=${page.search}'">50
        </button>
        <button type="button" class="btn btn-default"
                onclick="document.location.href='dashboard?id=${active}&size=100&search=${page.search}'">100
        </button>
    </div>
</div>